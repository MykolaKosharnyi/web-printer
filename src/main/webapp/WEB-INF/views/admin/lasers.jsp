...
<body>
	<div id="product">
	<a href="<c:url value='/admin/laser/new' />">Добавить лазер</a>
	<br>
	<h3>${titleOfTable}</h3>
		...
		<table class="tg">
			...
			<c:forEach items="${listProducts}" var="product">
				<tr id="${product.id}">
					<td>${product.id}</td>
					<td>${product.name}</td>
					<td><img src="/images/lasers/${product.id}/${product.pathPictures.get(0)}" alt=""></td>
					...
					<td><a href="<c:url value='/admin/laser/edit/${product.id}' />">Изменить</a></td>
					<td><a href="<c:url value='/admin/laser/remove/${product.id}' />">Удалить</a></td>
				</tr>
			</c:forEach>
		</table>
	</c:if>
	...
	function setShowOnHomePage(id, element){
		$.ajax({
			  type: 'post',
			  url: "/admin/laser/showOnHomePage/" + id,
			  data: JSON.stringify(element.checked),
			  contentType: "application/json; charset=utf-8",
              dataType: "json"
			  });	
...
function setShowOnLeftSide(id, element){
		$.ajax({
			  type: 'post',
			  url: "/admin/laser/showOnLeftSide/" + id,
			  data: JSON.stringify(element.checked),
			  contentType: "application/json; charset=utf-8",
              dataType: "json"
			  });	
