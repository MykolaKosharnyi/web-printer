...
<html lang="ru">
<head>
	<title>Цыфровые принтеры</title>
	<meta name="_csrf" content="${_csrf.token}"/>
    <meta name="_csrf_header" content="${_csrf.headerName}"/>
</head>
<body>
	<div id="product">
	<a href="<c:url value='/admin/digital_printer/new' />">Добавить новое оборудование</a>
	<br>
	<h3>${titleOfTable}</h3>
	<c:if test="${!empty listProducts}">
		<table class="tg">
			<tr>
				<th width="40">ID</th>
				<th width="120">Имя цыфрового принтера</th>
			...
			<c:forEach items="${listProducts}" var="product">
				<tr id="${product.id}">
					<td>${product.id}</td>
					<td>${product.name}</td>
					<td><img src="/images/digital_printers/${product.id}/${product.pathPictures.get(0)}" alt=""></td>
					...
					<td><a href="<c:url value='/admin/digital_printer/edit/${product.id}' />">Изменить</a></td>
					<td><a href="<c:url value='/admin/digital_printer/remove/${product.id}' />">Удалить</a></td>
				</tr>
			</c:forEach>
		</table>
	</c:if>
	...
	function setShowOnSite(id, element){
		$.ajax({
			  type: 'post',
			  url: "/admin/digital_printer/showOnSite/" + id,
	...
	function setShowOnHomePage(id, element){
		$.ajax({
			  type: 'post',
			  url: "/admin/digital_printer/showOnHomePage/" + id,
			  data: JSON.stringify(element.checked),
			  contentType: "application/json; charset=utf-8",
              dataType: "json"
			  });	
...
function setShowOnLeftSide(id, element){
		$.ajax({
			  type: 'post',
			  url: "/admin/digital_printer/showOnLeftSide/" + id,
			  data: JSON.stringify(element.checked),
			  contentType: "application/json; charset=utf-8",
              dataType: "json"
			  });	
