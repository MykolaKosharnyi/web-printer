...
<head>
	<style>
  		...
  		#sortable li img{ width:250px ; height:188px; margin: 0px 5px 5px 5px; }
  	</style>
	...
	<title>
		<spring:message text="Изменение картинки отображения подраздела " />
	</title>
</head>
<body>
	<c:url var="addPictures" value="/admin/pictures/menu/${directory}/${subDirectory}/upload_pictures" ></c:url>
			...
			<ul id="sortable">
				<c:forEach items="${listPictures}" var="namePicture">
					<li class="ui-state-default" id="${namePicture}">
						...
						<img src="/images/menu/${directory}/${namePicture}" alt="">
					</li>
				</c:forEach>
			</ul>
		</div>
      ...
      $(function() {
    	$('#sortable').sortable({
    		update: function(event, ui) {
    			...
    			$.ajax({
    				  type: 'POST',
    				  url: "/admin/pictures/menu/${directory}/${subDirectory}/change_order_pictures",
    				  data: data,
    ...
    $(document).on('click', '.delete_img', function(){
    	...
    	$.ajax({
    		  type: 'POST',
    		  url: "/admin/pictures/menu/${directory}/${subDirectory}/remove_picture/" + $(this).closest('li').attr('id').replace(".", ":"),
    		  contentType: "application/json; charset=utf-8",
              dataType: "json"
    		  });	
