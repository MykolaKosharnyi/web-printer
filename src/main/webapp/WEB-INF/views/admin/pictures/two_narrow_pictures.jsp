...
<style>
  ...
  #sortable li { 
  	...
  	font-size: 1em; 
  	text-align: center; }
  #sortable li img{ width:564px ; height:163px; margin: 0px 5px 5px 5px; }
  </style>
...
<body>
	<c:url var="addPictures" value="/admin/pictures/two_narrow_pictures/${directory}/${subDirectory}/upload_pictures" ></c:url>
	
	<div id="product">

		<label id="head_of_page"><spring:message text="${headOfPage}" /></label>
			
		<span>Ссылка прикрепленная к картинке:</span>	
			
		<c:if test="${listPictures.href=='#'}">
			<input type="text"  id="hrefOnPicture"  name="hrefOnPicture" value=""/>
		</c:if>
		<c:if test="${listPictures.href!='#'}">
			<input type="text"  id="hrefOnPicture"  name="hrefOnPicture" value="${listPictures.href}"/>
		</c:if>
			...
			<ul id="sortable">
				<c:forEach items="${listPictures.fileNameArray}" var="namePicture">
					<li class="ui-state-default" id="${namePicture}">
						...
						<img src="/images/home/two_narrow_pictures/${directory}/${subDirectory}/${namePicture}" alt="">
					</li>
				</c:forEach>
			</ul>
		</div>
      ...
      $(function() {
    	$('#sortable').sortable({
    		update: function(event, ui) {
    			...
    			$.ajax({
    				  type: 'POST',
    				  url: "/admin/pictures/two_narrow_pictures/${directory}/${subDirectory}/change_order_pictures",
    				  data: data,
    ...
    $(document).on('click', '.delete_img', function(){
    	...
    	$.ajax({
    		  type: 'POST',
    		  url: "/admin/pictures/two_narrow_pictures/${directory}/${subDirectory}/remove_picture/" + $(this).closest('li').attr('id').replace(".", ":"),
    		  contentType: "application/json; charset=utf-8",
              dataType: "json"
    		  });	
    	...
    	function saveDescpitpion(){
    		if($("#hrefOnPicture").val()==null || ($("#hrefOnPicture").val())==""){
    			$.ajax({
    				  type: 'POST',
    				  url: "/admin/pictures/two_narrow_pictures/${directory}/${subDirectory}/save_description/#",
    				  contentType: "application/json; charset=utf-8",
    		          dataType: "json"
    				  });		
    		}else{
    			$.ajax({
  				  type: 'POST',
  				  url: "/admin/pictures/two_narrow_pictures/${directory}/${subDirectory}/save_description/" + $("#hrefOnPicture").val(),
  				  contentType: "application/json; charset=utf-8",
  		          dataType: "json"
  				  });	
    		}
