...
<head>
	<style>
  		...
  		#sortable li img{ width: 260px ; height: 332px; margin: 0px 5px 5px 5px; }
  	</style>
...
<body>
	<c:url var="addPictures" value="/admin/pictures/home_products_block/${directory}/upload_pictures" ></c:url>
			...
			<ul id="sortable">
				<c:forEach items="${listPictures}" var="namePicture">
					<li class="ui-state-default" id="${namePicture}">
						...
						<img src="/images/home/product_block/${directory}/${namePicture}" alt="">
					</li>
				</c:forEach>
			</ul>
		</div>
      ...
      $(function() {
    	$('#sortable').sortable({
    		update: function(event, ui) {
    			...
    			$.ajax({
    				  type: 'POST',
    				  url: "/admin/pictures/home_products_block/${directory}/change_order_pictures",
    				  data: data,
    ...
    $(document).on('click', '.delete_img', function(){
    	...
    	$.ajax({
    		  type: 'POST',
    		  url: "/admin/pictures/home_products_block/${directory}/remove_picture/" + $(this).closest('li').attr('id').replace(".", ":"),
    		  contentType: "application/json; charset=utf-8",
              dataType: "json"
    		  });	
