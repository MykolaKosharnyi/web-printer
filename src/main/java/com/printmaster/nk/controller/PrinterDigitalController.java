...
public class PrinterDigitalController {
    ...
    public String listConcreteTypeProducts(@PathVariable("type") String type, Model model) {
		...
		List<DigitalPrinter> listResult = new ArrayList<DigitalPrinter>();
        
        if(links.containsKey(type)){
        	for(DigitalPrinter product : productService.listProducts("id")){
    ...
    public @ResponseBody List<DigitalPrinter> sortingProductsInAdmin(@PathVariable("type") String type, @PathVariable("value") String value) {
		...
		if (links.containsKey(type)) {
			...
			for (DigitalPrinter product : productService.listProducts(value)) {
				if (product.getTypeProduct().equals(links.get(type))) 
					list.add(product);
			}
	...
	public String addNewProduct(Model model) {
		...
		model.addAttribute(ATTRIBUTE_PRODUCT, new DigitalPrinter());
    ...
    public String editProduct(@PathVariable("id") long id, Model model){
    	...
    	DigitalPrinter undateProduct = productService.getProductById(id);
	...
	private String adminFormHasError(DigitalPrinter product, Model model){
		model.addAttribute(ATTRIBUTE_PRODUCT, product);
		model.addAttribute(ATTRIBUTE_UWP, componets.showSimplestArrayOfUseWithProduct(this.useWithProductService.listShowOnSite()));
		model.addAttribute(ATTRIBUTE_TYPE, TYPE);
