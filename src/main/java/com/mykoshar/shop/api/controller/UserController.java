...
import com.mykoshar.shop.api.validator.UserValidator;
...
import com.mykoshar.shop.api.model.service.SecurityService;
...
public class UserController {
  ...
  //for 403 access denied page
	...
	private UserAddByAdmin.StatusSubscription setStatusOfSubscriptionC(int sizeForm, int sizeCurrent, int sizeCheckedProperty){
		UserAddByAdmin.StatusSubscription result = UserAddByAdmin.StatusSubscription.NOT_CHANGED;
	
		if(sizeForm==0 && sizeCheckedProperty==0){
			result = UserAddByAdmin.StatusSubscription.COMPLETELY_UNSUBSCRIBED;
		} else if(sizeForm>sizeCurrent){
			result = UserAddByAdmin.StatusSubscription.ADDED_SUBSCRIPTIONS;
		} else if(sizeForm<sizeCurrent){
			result = UserAddByAdmin.StatusSubscription.REMOVED_SOME_SUBSCRIPTIONS;
		}
